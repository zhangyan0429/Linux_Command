# 命令
 ...
 - 基本命令
  ...
  - viエディター
    ...
    + ソフトウェアとパッケージ
        + apache HTTP Serverのコンパイルとインストール
          + tar xzvf httpd-2.2.14.tar.gz    アーカイブを展開
          + cd httpd-2.2.14
          + ./configure configureスクリプトの実行
          + make
          + su
          + make install
        + パッケージの管理
          + RPMとYUM
            + rpm　オプション
              ...
              + 例：rpm -qa
                    ...
                    rpm -q postfix
                    rpm -q httpd
                    rpm -ivh [rpmパッケージファイル]　　パッケージをインストール
                    rpm -Uvh [rpmパッケージファイル]    アップグレード
        ...
        + ファイルシステム
            + デバイスファイル
              + ls　/dev    udevという仕組みによって自動生成されます
            + パーティションの作成
              + パーティション
                + 基本パーティション４つ    /dev/sda1,2,3,4
                + 拡張パーティション（複数の論理パーティション作成できます）    /dev/sda5,6
              + 操作
                + fdisk デバイスファイル名
            + ファイルシステムの作成
              + 主なファイルシステムの種類
              ...
              + マウントとアンマウント
                + マウント
                  + mount デバイスファイル　マウントポイント（任意ディレクトリ）
        ...
        + プロセスの管理
          + ps [オプション] プロセスの情報を表示 
            ...
            + PSコマンドの表示項目
              ...
              + TIME    プロセスの実行時間
              + CMD     実行コマンド
            + PStree　[オプション]  プロセスの親子関係を表示
            + top   プロセスの実行状況を確認、3秒間隔で更新
            + q     終了
          + プロセスの終了
            + シグナル
              + 主なシグナル
              ...
              + シグナル送信
                ...
                + kill -s [シグナル名]　プロセスID
                  + kill -s TERM 28000
                + killall　-15　xclock  プロセス名を指定して終了する
        + ジョブ
            + バックグラウンド
              + less /etc/services &
              + [1] 28060   ジョブIDとプロセスID
            + ジョブ操作
              + jobs     実行中のジョブを確認
              + fg　[%ジョブID] ジョブをフォアグランドで実行
              + fg　%3
              + ctrl + z    フォアグラウンドでジョブが実行されている最中にそのジョブを一時停止する
              + bg　[%ジョブID]  一時停止中のジョブをバックグラウンドで再開
        + ネットワーク
            + ポート
              + 20  FTPデータ転送
              + 21  FTP制御情報
            ...
            + ネットワーク設定
              + /etc/hostsファイル          IPアドレスとホスト名対応
              + /etc/resolv.confファイル    DNSサーバ
              + /etc/sysconfig/networkファイル  ネットワーク基本情報（RedHat系固有）
              + /etc/sysconfig/network-scripts/ifcfg-eth0ファイル   ネットワークインターフェース設定
              + ネットワークの再起動
                + service network restart
            + ネットワークを利用するコマンド
              + Telent　接続先ホスト名　またはIPアドレス    別のコンピュータにログイン
                + Telent　192.168.0.10
              + SSH　[ユーザー名＠]　接続先ホスト名 またはIPアドレス
                + SSH　penguin@192.168.0.10
                + exit
              + ping
